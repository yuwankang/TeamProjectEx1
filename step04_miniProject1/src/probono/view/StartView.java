package lunch.view;

/** 
4 * PROJECT  : 점심 가게
 * NAME  :  StartView.java
 * DESC  : 실행 클래스
 * 		     기부자, 수혜자, 재능기부 종류 및 실제 진행되는 프로젝트 생성 및 CRUD 로직을 test하는 클래스
 */

import lunch.controller.LunchProjectController;
import lunch.model.dto.food;
import lunch.model.dto.team;
import lunch.model.dto.LunchProject;
import lunch.model.dto.LunchProjectType;

public class StartView {

	public static void main(String[] args) {
		// 기부자 정보 - 사번, 사원명, 이메일, 기부하기를 희망하는 재능기부 종류
		Team team1 = new Team("1팀", "BTS", "아이유", "블랙핑크", "김범수", "양식");
		Team team2 = new Team("2팀", "손흥민", "황희찬", "이강인", "김민재", "중식");
		Team team3 = new Team("3팀", "유재석", "박명수", "정형돈", "정준하", "일식");
		Team team4 = new Team("4팀", "강호동", "이수근", "김종민", "은지원", "한식");
		Team team5 = new Team("5팀", "강유완", "곽병찬", "구동길", "김상민", "패스트푸드");

		// 음식 종류
		Food food1 = new Food("양식", "파스타, 피자");
		Food food2 = new Food("중식", "짜장면, 마라탕");
		Food food3 = new Food("일식", "초밥, 라멘");
		Food food4 = new Food("한식", "김치찌개, 제육볶음");
		Food food5 = new Food("패스트푸드", "맥도날드, 맘스터치");

		// 가게 목록
		LunchProjectType Western = new LunchProjectType("양식", " ", " ");
		LunchProjectType China = new LunchProjectType("중식", " ", " ");
		LunchProjectType Japan = new LunchProjectType("일식", " ", " ");
		LunchProjectType Korean = new LunchProjectType("한식", " ", " ");
		LunchProjectType Fastfood = new LunchProjectType("패스트푸드", " ",	" ");

		// 기부자, 수혜자가 매핑된 실제 진행되는 "재능 기부 프로젝트"
		// 프로젝트명, 팀, 음식, 음식종류, 시작시간, 종료시간, 특이사항
		LunchProject WesternProject
		= new LunchProject("양식", team1, food1, Western, "13:00~", "2024-12-03"," ");
							        		
		LunchProject ChinaProject
		= new LunchProject("중식", team2, food2, China, "13:00~", "2024-12-03", " ");
		
		LunchProject JapanProject 
		= new LunchProject("일식", team3, food3, Japan, "13:00~", "2024-12-03", " ");
		
		LunchProject KoreanProject 
		= new LunchProject("한식", team4, food4, Korean, "13:00~", "2024-12-03", " ");
		
		LunchProject FastfoodProject 
		= new LunchProject("패스트푸드", team5, food5, Fastfood, "13:00~", "2024-12-03", " ");
		  

		// 데이터 구성 후 서비스 로직 실행
		LunchProjectController controller = LunchProjectController.getInstance();
		

		System.out.println("*** 01. Project 생성 ***");
		// 저장 시도시 이미 존재할 경우 예외 발생, 미 존재시 정상 저장 실행
		controller.lunchProjectInsert(WesternProject);
		controller.lunchProjectInsert(ChinaProject);
		controller.lunchProjectInsert(JapanProject);
		controller.lunchProjectInsert(KoreanProject);
		controller.lunchProjectInsert(FastfoodProject);

		
		System.out.println("\n*** 02. 모든 Project 검색 ***".toString());
		controller.getLunchProjectsList();
		

		System.out.println("\n*** 03. '한식' Project 검색 ***");
		controller.getLunchProject("한식");
		

		//한식 변경하기
		System.out.println("\n*** 04. '한식' Project의 기부자 변경(수정) 후 해당 Project 검색 ***");
		controller.lunchProjectUpdate("한식", team5);
		controller.getLunchProject("한식");
		
		
		//한식 삭제하기 
		System.out.println("\n*** 05. '한식' Project 삭제 후 삭제한 Project 존재 여부 검색 ***");
		controller.lunchProjectUpdate("한식");
		controller.getLunchProject("한식");
		
	}
}
